...
class HunyuanRenderer():
    ...
    def render(self, image_size, R, T, K=None, focal_length=None, principal_point=None, blur_radius=0.0):
        ...
        # Light follows the camera so every azimuth/elevation is consistently lit.
        # Without this the fixed world-space light makes back-views nearly black,
        # completely defeating VGGT-Omega's cross-view correspondence.
        # A 40% ambient term ensures shadowed surfaces stay visible too.
        cam_pos_list = cam_pos.tolist()
...
class CameraPoseFinder:
    ...
    def _apply_flux_texture_transfer(
        ...
        # Tight, geometry-preserving texture-transfer prompt.
        # Rules to minimise hallucination:
        #   1. Explicitly names each image's role so the model is unambiguous.
        #   2. Every geometric dimension that must NOT change is listed.
        #   3. guidance_scale=3.0 (was 1.0 ≈ unconditioned → random per seed).
        #   4. num_inference_steps=8 for coherence without drifting.
        # prompt = (
        #     "Image 1 is a 3D-rendered mesh on a white background. "
        #     "Image 2 is a reference photo that provides the target surface appearance. "
        #     "Task: repaint the surface of the object in Image 1 to match the texture, "
        #     "color palette, and material look of the object in Image 2. "
        #     "Do NOT change: the silhouette, body shape, 3D pose, camera viewpoint, "
        #     "proportions, white background, or any part of the composition from Image 1. "
        #     "Do NOT add objects, shadows, or elements not present in Image 1. "
        #     "Output: Image 1's geometry and layout with Image 2's surface colors and "
        #     "texture patterns applied to the object surface — nothing else modified."
        # )
        prompt = (
            "Improve only the surface texture detail of the object in image 1 — "
            "Preserve color palette and shading and focus on the light. "
            "sharpen and enrich material qualities like roughness, fabric weave, "
    ...
    def get_vggt_initial_guess(self, target_image_path, all_Rs, all_Ts, img_name):
        ...
        # Convert every context view and the target to grayscale-as-RGB before
        # feeding VGGT-Omega.  The model estimates pose from geometry/structure
        # (edges, silhouette, surface normals) — colour is pure noise here and
        # actively hurts: the rendered views and the real photo have different
        # colour temperatures, material renders, and Flux processing artefacts.
        # Stripping colour removes that domain gap so VGGT attends to shape only.
        grey_dir = os.path.join(self.view_path, "_grey_vggt")
        os.makedirs(grey_dir, exist_ok=True)
        grey_image_paths = []
        for p in all_image_paths:
            grey_p = os.path.join(grey_dir, os.path.basename(p))
            Image.open(p).convert("L").convert("RGB").save(grey_p)
            grey_image_paths.append(grey_p)

        images = load_and_preprocess_images_omega(grey_image_paths, image_resolution=512).to(self.device)

        print("Running VGGT-Omega for initial pose estimation (grayscale mode)...")
        with torch.inference_mode():
