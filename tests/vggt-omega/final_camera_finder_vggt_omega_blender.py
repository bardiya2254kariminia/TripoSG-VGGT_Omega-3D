...
blending_utils_path = os.path.abspath(os.path.join(current_dir, ".."))
...
class CameraPoseFinder:
    ...
    def generate_initial_views(self, num_views, img_name):
        """
        Generate initial views as RGB renders (full color with textures).
        These are fed directly to VGGT-Omega for pose estimation.
        """
        print(f"Generating {num_views} initial RGB views for context...")
        ...
        for azimuth in azimuths:
            for elevation in elevations:
                ...
                # Render full RGBA and save as RGB (no silhouette conversion)
                ...
                # Extract RGB channels directly — feed color to VGGT-Omega
                rendered_rgb = rendered_rgba[..., :3]
                rendered_rgb = np.clip(rendered_rgb, 0.0, 1.0)

                save_path = f'temp_views/{img_name}_{num_of_views}.png'
                plt.imsave(save_path, rendered_rgb)
                self.known_image_paths.append(save_path)
                num_of_views += 1

        print(f"Generated {num_of_views} RGB views.")
    ...
    def get_vggt_initial_guess(self, target_image_path, all_Rs, all_Ts, img_name):
        ...
        target_rgb_path = f"temp_views/_rgb_target_{img_name}.png"
...
output_path = f"/workspace/rendered_images/{output_filename}"
